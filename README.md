...
- [x] create `src/server/auth/index.ts` for better auth instance setup
...
- [x] setup prisma with `bunx prisma init --datasource-provider postgresql --output ../src/orm`
...
- [x] create `src/server/db.ts` for global prisma instance setup
- [x] setup prisma adapter with better auth using `src/server/auth/index.ts`
- [x] generate auth tables using `bun x @better-auth/cli@latest generate --config ./src/server/auth/index.ts --output=./prisma/auth.prisma --yes`
...
- [x] replace auth import in mount handler with `import { auth } from "@/server/auth";`
- [x] create better auth client instance in `src/server/auth/client.ts`
